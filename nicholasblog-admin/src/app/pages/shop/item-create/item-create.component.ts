...
import { itemCreateLib } from './item-createlib';
import { Item } from './item-createlib';
import { Option } from './item-createlib';
declare var $: any;
...
@Component({
  ...
  styleUrls: [
    './item-create.component.scss',
    '../../../lib/dropzone.css'
  ],
  providers: [ItemCreateService]
})

export class ItemCreateComponent implements OnInit, DoCheck, OnDestroy {
  ...
  @ViewChild('uploadItem') uploadItemButton;
  ...
  statusMethods = {
    save: function () {
      this.itemTemp.dateid = this.dateid
      Object.assign(this.itemTemp, { item_option: this.item_option });
      execLib.uploadexec.call(this,
        this.service.updateTemp(this.itemTemp),
        function () {
    ...
    upload: function () {
      ...
      Object.assign(this.itemTemp, { item_option: this.item_option });
      Object.assign(this.item, this.itemTemp);
      ...
      execLib.uploadexec.call(this,
        this.service.uploadItem(this.item),
        function () {
  ...
  constructor(
    private service: ItemCreateService,
  ...
  ngOnInit() {
    ...
    this.updateEnable.add(this.uploadItemButton.nativeElement);
    this.loadImage();
    this.getTemp();
    this.config = {
      url: `${execLib.apiName}shop/createSaveImage`,
      uploadMultiple: true,
      maxFiles: 4,
      ...
      init: function () {
        ...
        this.on('thumbnail', function () {
          ...
          $('.dz-remove').css(itemCreateLib.removeclass);
        });
          ...
          if (that.finish == false) {
            if (file.path) {
              if (that.uploadError.boolean) {
                ...
                const index = _.findIndex(that.item_img, { filename: { filename: file.filename } });
    ...
    this.options = {
      ...
      emoticonsSet: itemCreateLib.emoticonsSet
    };
  }
  ...
  public gets(event): void {
    ...
    switch (true) {
      case id == 'updateTemp' || parent == 'updateTemp':
        ...
        break;
      case id == 'uploadItem' || parent == 'uploadItem':
        exec.call(this, this.uploadItem);
        break;
  ...
  selectNumber(e) {
    ...
    if (this.selects <= 0) {
      ...
      if (this.selectItems.length === Number(this.selects)) {
  ...
  public loadImage(): void {
    execLib.getexec.call(this,
      this.shopService.createLoadImage({ dateid: this.dateid }),
      function (res) {
  ...
  getTemp() {
    execLib.getexec.call(this,
      this.shopService.getTemp(),
      function (res) {
        const that = this;
        const { _id, item_name, unit_price, item_detail, item_option } = res.data[0];
        Object.assign(this.itemTemp, { _id, item_name, unit_price, item_detail, item_option });
  ...
  upData(status) {
    let statusArray = [];
    if (!this.itemTemp.item_name || this.itemTemp.item_name.length === 0 || this.itemTemp.item_name.replace(/(^s*)|(s*$)/g, '').length === 0) {
      alert('商品名称不能为空');
    } else if (!this.itemTemp.unit_price || this.itemTemp.unit_price.length === 0 || this.itemTemp.unit_price.replace(/(^s*)|(s*$)/g, '').length === 0) {
      alert('商品单价不能为空');
    } else if (isNaN(Number(this.itemTemp.unit_price))) {
      alert('商品单价必须为数字');
    } else if (!this.itemTemp.item_detail || this.itemTemp.item_detail.length === 0 || this.itemTemp.item_detail.replace(/(^s*)|(s*$)/g, '').length === 0) {
      alert('商品介绍不能为空');
